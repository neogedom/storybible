---
type: VisualState
title: Dagmar — Timeline Visual
character: Dagmar
tags: [visual, timeline, dagmar]
timestamp: 2026-07-24T12:00:00-03:00
---

## Estado Canônico (Linha de Base)

| Atributo   | Valor                                        |
| ---------- | -------------------------------------------- |
| Cabelo     | Claro (louro/castanho-claro), preso ou solto |
| Pele       | Clara                                        |
| Roupas     | Vestidos de viagem de corte nobre            |
| Acessórios | Nenhum                                       |
| Armas      | Nenhuma                                      |
| Cicatrizes | Nenhuma                                      |
| Gravidez   | Ventre proeminente (grávida de Saboc)        |
| Expressão  | Mágoa, tensão                                |

## Capítulo 0 — Prólogo

- _Não aparece_

## Histórico por Capítulo

<!-- Atualizado a cada INGEST de capítulo -->

---

## Prompt de Imagem (modelo)

`Dagmar, cabelo claro, pele clara, grávida (ventre proeminente), [roupas do capítulo], expressão de [emoção], iluminação [tipo], fundo [descrição] — tons #A8C4D8, #E8DCC8, #C4A882, #6B5B4F, #8B0000`
