---
type: Theme
title: Outline da Série (Save The Cat!)
description: Índice de referência rápida por estágio STC para todos os livros.
tags: [stc, outline, referencia, L1, L2]
timestamp: 2026-07-25T12:00:00-03:00
---

# Story Bible — Outline da Série (Save The Cat!)

> **Resumo rápido por estágio STC.** Para roteiro cap-a-cap completo, consultar:

- `temas/esquema-l1.md` — Livro 1 (fonte primária, cap por cap com % recalibradas para multi-PoV)
- `temas/esquema-l2.md` — Livro 2 (fonte primária, por estágio STC)
- `temas/arco-stc.md` — Visão geral dos beats STC com progressão por capítulo
- `conceitos/riscos-estruturais.md` — Mapeamento das 3 colisões e 5 riscos estruturais
- `memories/repo/arquiteto-densidade-avaliacao-l1.md` — Diagnóstico completo do desvio de pacing e tabela recalibrada

---

## Regra de Mapeamento

Cada capítulo deve responder:

1. **Qual estágio STC ocupa?** (0-100%)
2. **Qual promessa do estágio anterior deve ser cumprida aqui?**
3. **Qual arco de personagem está ativo?**
4. **Qual colisão (Mário↔Nolan, Mário↔Kate, Nolan↔Kate) está presente?**

---

## Livro 1 — Progressão dos Beats (recalibrado 2026-08-26)

> **Nota de recalibragem:** As % abaixo foram ajustadas para refletir o word count real do L1 (~115.000 palavras estimadas). O desvio médio é de ~+10 pontos em relação ao planejado original — o L1 está mais longo, mas o ritmo narrativo está saudável. A janela de cada beat foi expandida para acomodar o pacing real.

| Estágio STC              | % (recalibrado) | Capítulos | Status       |
| ------------------------ | --------------- | --------- | ------------ |
| Imagem de Abertura       | ~0-3%           | 0         | ✅ Escrito   |
| Configuração             | ~3-7%           | 1-2       | ✅ Escrito   |
| Catalisador              | ~7-12%          | 3-4       | ✅ Escrito   |
| Debate                   | ~12-42%         | 4-18      | ✅ Escrito   |
| Break Into 2             | ~42-45%         | 19-20     | ✅ Escrito   |
| Jogos (Velocino de Ouro) | ~45-68%         | 21-35     | ✍️ Escrevendo |
| Ponto Central            | ~68%            | ~34-35    | 📝 Planejado |
| Vilões se Aproximam      | ~68-75%         | —         | 📝 Planejado |
| Tudo Está Perdido        | ~75%            | —         | 📝 Planejado |
| Noite Escura da Alma     | ~75-80%         | —         | 📝 Planejado |
| Break Into 3             | ~80%            | —         | 📝 Planejado |
| Final                    | ~80-99%         | —         | 📝 Planejado |
| Imagem Final             | ~99-100%        | —         | 📝 Planejado |

> **Projeção:** Mantido o ritmo atual (~2.600 palavras/capítulo), o Ponto Central (~68%) chegará por volta do cap. 33-34 com ~80.000 palavras acumuladas. O L1 total deve ficar entre 110.000 e 120.000 palavras.

---

## Livro 2 — Visão Geral

Consultar `temas/esquema-l2.md` para o outline completo.
